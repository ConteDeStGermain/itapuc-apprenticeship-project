...
const Main = ({ apiHost, session, onLogout }) => {
  ...
  const currentRoomId = useMemo(
    () =>
      rooms
        .filter((r) => r.participants.includes(chattingUserId))
        .map((r) => r.id)[0],
    [rooms, chattingUserId]
  );
    ...
    try {
      ...
      if (usersResponse.ok) {
        setUsers(
          await usersResponse
            .json()
            .then(({ data }) => data.filter((u) => u.id !== session.user.id))
        );
      } else {
  ...
  return (
    <Grid style={{ height: "100vh" }}>
      <Grid.Column
        width="4"
        style={{ height: "100%", borderRight: "5px solid grey" }}
      >
        <Grid.Row>
          <Button
            style={{ width: "49%" }}
          ...
          <List celled>
            {selectedTab === "Users"
              ? users.map((user) => (
                  <List.Item
                    key={user.id}
                    onClick={() => setChattingUserId(user.id)}
                  >
                    <List.Content>
